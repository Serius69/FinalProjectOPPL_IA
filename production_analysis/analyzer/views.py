...
def improve_efficiency(request):
    # Obtener los parámetros de la solicitud (POST en vez de GET: esta vista muta datos)
    logistic_process_id = request.POST.get('process_id', 1)
    budget = float(request.POST.get('budget', 100000))
    efficiency_improvement_value = float(request.POST.get('efficiency_improvement', 10))  # Mejora en eficiencia
